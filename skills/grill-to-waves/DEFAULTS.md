...
- **Executors: three tiers, chosen by the consequence and reach of an incorrect implementation.** The
  mid tier (Sonnet) for bounded work whose decisions were all made upstream; the frontier tier (Opus)
  for judgment inside a bounded blast radius; and the Fable tier where failure is irreversible,
  run-wide or adversarial. Follow the ticket's `executor:` label where one exists.
...
- **The orchestrator and the verifier run on the most capable model available — Fable, then
  Opus.** Verification of executor output is the orchestrator's job, never left to the
...
|---|---|---|
| `medium` | The target is fully specified upstream and the exploration is narrow: the files are named or trivially found, the edit is mechanical, and no decision is left to make — a rename, a dead column removed at enumerated sites, a copy change, a docs mirror, a restyle onto a settled component. | Usually Sonnet `medium`; any tier, Fable included, may be `medium` when its tier criteria hold but its work is fully specified |
...
| `xhigh` | Judgment *is* the work: deriving a correctness argument, invariant, migration or security strategy, concurrency proof, or run-wide contract rather than implementing one already settled. | Sonnet `xhigh` is legal but rare; Opus `xhigh` for a bounded blast radius; Fable `xhigh` when its tier criteria hold |
...
- **Fallback.** Where the matching `executor-fable-<effort>` is missing or the Fable model is not
  available on the account, dispatch `executor-opus-<effort>` with the same brief, keep both labels, and record the
...
fall back to `executor-opus-<effort>`. If any other agent is missing, use the host's
